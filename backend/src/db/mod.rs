...
async fn get_client_handle() -> Client {
  ...
  let mut options = ClientOptions::parse(&mongo_url).await.expect("Log: Unable to parse MONGO_URI!");
  ...
  let client = Client::with_options(options).expect("Log: Unable to create client!");
  return client;
}
...
pub async fn get_db_handle() -> Database {
  ...
  return match test_mode {
    Ok(_) => get_client_handle().await.default_database().expect("Log: Unable to get default database!"),
    Err(_) => CLIENT.get().await.default_database().expect("Log: Unable to get default database!")
  };
}
